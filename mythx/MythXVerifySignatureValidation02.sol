...
contract SignatureValidator {
  ...
  uint256 n_valid_signers;
  uint256 recovered_signers;
...
abstract contract ModuleAuthFixed is ModuleAuth {
  ...
  /**
   *  Verify that signature verification does not pass if the cumulative weight of signers is lower than the threshold
   *  (for any given threshold and signature data up to 512 bytes length)
  */

 
contract MythXVerifySignatureValidation02 is ModuleAuthFixed {

  using LibBytes for bytes;

  event AssertionFailed(string message);

  bytes32 HASH_TO_SIGN = 0xd7d870b04211a7469289e966fe1e3cfbfbd87733720492ef04397c393e40a902; // random
  ...
  function shouldNotPassSignatureCheck(
    ...
    if (_signatureValidation(HASH_TO_SIGN, input)) {
        emit AssertionFailed("[P14] Signature validation must always return false if cumulative weight < threshold.");
    }
  }
