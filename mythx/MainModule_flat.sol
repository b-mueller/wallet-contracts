...
contract ModuleHooks is IERC1155Receiver, IERC721Receiver, IModuleHooks, ModuleERC165, ModuleSelfAuth, MythXVerification {
  ...
  */
   ...
   * @param _implementation Hook implementation contract
   */
  function addHook(bytes4 _signature, address _implementation) external checkInvariants override onlySelf {
   ...
   * @param _signature Signature function linked to the hook
   */
  function removeHook(bytes4 _signature) external override onlySelf checkInvariants {
  ...
  */
  ...
  function onERC1155Received(
    ...
    uint256,
    bytes calldata
  ) external override checkInvariants returns (bytes4) {
   ...
   */
  function onERC721Received(address, address, uint256, bytes calldata) external override checkInvariants returns (bytes4) {
   ...
   */
  fallback() external payable checkInvariants {
...
abstract contract ModuleCalls is IModuleCalls, IModuleAuth, ModuleERC165, ModuleSelfAuth, MythXVerification {
  ...
  function execute(
    ...
    uint256 _nonce,
    bytes memory _signature
  ) public override checkInvariants {
  ...
  function selfExecute(
    Transaction[] memory _txs
  ) public override onlySelf checkInvariants {
    // Hash transaction bundle
...
contract ModuleUpdate is IModuleUpdate, ModuleERC165, ModuleSelfAuth, Implementation, MythXVerification {
   ...
   * @dev WARNING Updating the implementation can brick the wallet
   */
  function updateImplementation(address _implementation) external override onlySelf checkInvariants {
